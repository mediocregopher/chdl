...
(def ports (apply comp/port (concat
  (apply concat (map #(identity [%1 :in :BIT]) in-sigs))
  (apply concat (map #(identity [%1 :out :BIT]) out-sigs)))))

(def assigns (map
  #(let [[[in1 in2] out] %1]
        (comp/assign out (math/xor (lit/raw in1) (lit/raw in2))))
